...
import { buildReport, classifySource, countCacheHits, postReport } from "../src/telemetry";
...
describe("buildReport", () => {
  it("produces a schema-2 report that validates against schemas/telemetry.v2.json", () => {
    ...
    const r = buildReport({ model: "qwen2.5-0.5b-instruct", variant: "q4", bytes: 323893760, sources: ["network", "per-site-cache", "per-site-cache"], ms: 1234.6, verifyMs: 88.2, transferMs: 0, cacheMode: "per-site", device });
  ...
  it("omits quota and max_buffer_size when the device does not report them", () => {
    const r = buildReport({ model: "m", variant: "fp16", bytes: 0, sources: [], ms: 0, verifyMs: 0, transferMs: 0, cacheMode: "none", device: { ...device, quotaBytes: null, maxBufferSize: null } });
